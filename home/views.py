...
def AddBookSubmission(request):
    if request.session.has_key('is_logged'):
        if request.method == "POST":
            ...
            publishinghouse = request.POST["publishinghouse"]
            year = request.POST["year"]
            subject = request.POST["subject"]
            category=request.POST["category"]
            keywords=request.POST["keywords"]
            add = AddBook(user = user1,bookid=bookid,bookname=bookname,author=author,publishinghouse=publishinghouse,year=year,subject=subject,category=category,keywords=keywords)
            add.save()
            Book = AddBook.objects.all()
            return render(request,'dashboard.html',{'Book':Book})
    return redirect('/')
...
def updatedetails(request,id):
    if request.session.has_key('is_logged'):
        if request.method=="POST":
                ...
                add.publishinghouse=request.POST["publishinghouse"]
                add.year=request.POST["year"]
                add.subject=request.POST["subject"]
                add.ContactNumber=request.POST['category']
                add.keywords=request.POST["keywords"]
                add.save()
                return redirect("dashboard")
    return redirect('login')
...
def viewissuedbook(request):
    if request.session.has_key('is_logged'):
        ...
        for books in issuedbooks:
            ...
            for k in book:
                print(li)
                t=(students[i].sname,students[i].studentid,book[i].bookname,book[i].subject,book[i].keywords,issdate,expdate,fine)
                print(t)
                i=i+1
                lis.append(t)
                print(lis)
